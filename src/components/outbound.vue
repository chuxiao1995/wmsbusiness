        ...
        <section class="main">
          <el-row class="tableCont notftr bor_ff bk_ff">
            ...
            >
              ...
              <el-table-column prop="ioWhSortCodeValue" label="出库类型" width="220"></el-table-column>
...
import manualOrder from "./ioorder/out manualorder";
import autOrder from "./ioorder/out autorder";
// import { async } from "q";
export default {
  //在components声明
  components: {
  ...
  data() {
    return {
      ...
      formData: {
        ...
        inStoreTypeArrays: [
          { name: "采购出库", value: "0101" },
          { name: "调拨出库", value: "0103" },
          { name: "废旧退货出库", value: "0104" }
        ],
        storeNameArrays: [
          { name: "库房1", value: "01" },
          { name: "库房2", value: "02" }
  ...
  methods: {
    ...
    async startOneTask(val) {
      ...
      await this.axios
        .post("/wmsbusiness/iotask/runOutboundTask", val)
        .then(res => {
          console.log(res.data);
          if (res.data == 0) {
            this.$notify({
              title: "重要",
              message: val.ioTaskNo + "出库任务，库存容量无法满足",
              type: "warning",
              duration: 0
            });
          } else if (res.data == 1) {
            ...
            this.$notify({
              title: "重要",
              message: val.ioTaskNo + "出库任务需要人工出库,数量为" + s[1],
              type: "warning",
              duration: 0
            });
          }
          ...
          if (this.showTable2 == true) {
            ...
            this.$refs.manual.initIoOrderTable();
          }
        })
        .catch(err => {
          this.$notify.error({
